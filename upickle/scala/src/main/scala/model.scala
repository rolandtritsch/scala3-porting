...
object model {
  ...
  case class SingularityTask(
      taskId: SingularityTaskId,
      mesosTask: SingularityTask.SingularityTaskInfo,
      offer: Offer
  )
  ...
  object SingularityTask {
    ...
    case class SingularityTaskContainerInfo(
        docker: SingularityTaskDockerInfo
    )

    object SingularityTaskContainerInfo {
      implicit val rw: RW[SingularityTaskContainerInfo] = macroRW
    }
